...
from rpc_client import request
...
def game_on(state, options):
    print_status(state, options)
    selection = input("Please Select:")
    if selection == '1':
        state = request({'type': 'play', 'payload': play(
            state['user_board_history'][-1])})
    elif selection == '2':
        state = request({'type': 'pause'})
    elif selection == '3':
        state = request({'type': 'quit'})
    else:
...
def start():
    request({'type': 'new_game'})
    class Game:
        def play(self):
            options = menu.keys()
            options = sorted(options)

            state = request({'type': 'get_status'})
            status = state['status']
            game_on(state, options)
